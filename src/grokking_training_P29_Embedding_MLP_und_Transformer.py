...
# ------------------------------------------------------------
P = 29
EPOCHS_CONTINUE = 30000     # wie lange wir noch *weiter* trainieren es sei den wir beginnen von anfang an
BATCH_SIZE = 4              
LR = 0.001               #0.05 für SGD, aber = 0.001 für Transformer & AdamW (Viel stabiler )
                        
WEIGHT_DECAY = 1.8    # kommt für AdamW optimzer, nicht für SGD, nach Epoche 30_000 habe ich auf 0.8 erhoht
SAVE_EVERY = 1000
KEEP_LAST = 3
train_frac = 0.35      ## 35 % der Daten nutzen (bewährter Wert aus der Forschung) P=29
...
RESUME_RUN = "./runs/grok_P29_20260125_024425" 
...
def build_tiny_transformer_model(P, HIDDEN_SIZE, WEIGHT_DECAY):
    ...
    model.compile(
        optimizer=tf.keras.optimizers.AdamW(learning_rate=0.001, weight_decay=WEIGHT_DECAY),
        loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
...
#     # 1. Embedding (Basis für beide)
#     x = tf.keras.layers.Embedding(input_dim=P, output_dim=HIDDEN_SIZE)(inputs)
    
#     # 2. TRANSFORMER TEIL (Interaktion zwischen a und b)
#     attn_out = tf.keras.layers.MultiHeadAttention(num_heads=4, key_dim=16)(x, x)
#     x = tf.keras.layers.Add()([x, attn_out])
#     x = tf.keras.layers.LayerNormalization()(x)
    
#     x = tf.keras.layers.Flatten() # Vorbereitung für MLP
    
#     # 3. MLP TEIL (Deine ursprüngliche Struktur)
...
if checkpoints:
    ...
    if USE_TRANSFORMER:
        print("Baue: TINY TRANSFORMER")
        model = build_tiny_transformer_model(P, HIDDEN_SIZE, WEIGHT_DECAY)
    else:
        print("Baue: IMPROVED MLP")
        model = build_mlp_with_embeddings(P, HIDDEN_SIZE, WEIGHT_DECAY)
        # model = build_mlp_with_embeddings() # Erstelle neues Modell nur bei Bedarf
